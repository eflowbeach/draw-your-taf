...
tafout = '/awips/adapt/avnfps/OB9.2/tmp/'
tafdir2 = '/awips/adapt/avnfps/OB9.2/tmp/drawyourtaf'#taf.4.xt3-pqr'
...
# Cig Canvas #############		
		# top and bottom
		self.c.create_line(pct_width_laxis*width, pct_top*height,pct_width_raxis*width, pct_top*height ,fill=axis_color)
		self.c.create_line(pct_width_laxis*width, pct_bot*height,pct_width_raxis*width, pct_bot*height ,fill=axis_color)
		
		#left and right
		self.c.create_line(pct_width_laxis*width, pct_top*height,pct_width_laxis*width, pct_bot*height ,fill=axis_color)
		self.c.create_line(pct_width_raxis*width, pct_top*height,pct_width_raxis*width, pct_bot*height ,fill=axis_color)
		...
		for i in range(25):
			(year,month,tafday,tafhour,tmin,jsec,wday,yday,dst) = time.gmtime(time.mktime(self.tafpackage)+i*3600)
			#tafhour = self.tafphour + i		
			#time and vertical lines/X-axis Legend
			self.c.create_text(i*pct_graph_width*width/24+pct_width_laxis*width,height*0.05,text=tafhour,fill=axis_color)
			self.c.create_line(i*pct_graph_width*width/24+pct_width_laxis*width, pct_top*height,
			i*pct_graph_width*width/24+pct_width_laxis*width,pct_bot*height,fill=axis_color,dash=(4,4))
			
		for index,i in enumerate(range(len(heights))):
			j = float(heights[i])*100
			self.c.create_text(pct_width_laxis*width-20,abs(graph_height-(graph_height*(math.log10(j)-2)/2))+pct_top*height,text=heights[index],fill=axis_color)
...
# VIS CANVAS ##########################
		...
		# top and bottom
		self.cv.create_line(pct_width_laxis*width, pct_top*height_vis,pct_width_raxis*width, pct_top*height_vis ,fill=axis_color)
		self.cv.create_line(pct_width_laxis*width, pct_bot*height_vis,pct_width_raxis*width, pct_bot*height_vis ,fill=axis_color)
		
		#left and right
		self.cv.create_line(pct_width_laxis*width, pct_top*height_vis,pct_width_laxis*width, pct_bot*height_vis ,fill=axis_color)
		self.cv.create_line(pct_width_raxis*width, pct_top*height_vis,pct_width_raxis*width, pct_bot*height_vis ,fill=axis_color)
		...
		#VFR
		self.cv.create_rectangle(pct_width_laxis*width, pct_top*height_vis,pct_width_raxis*width, y2 ,fill="#A9FFA6")
	...
	# x-axis set-up
		for i in range(25):
			(year,month,tafday,tafhour,tmin,jsec,wday,yday,dst) = time.gmtime(time.mktime(self.tafpackage)+i*3600)		
			#time and vertical lines/X-axis Legend
			self.cv.create_text(i*pct_graph_width*width/24+pct_width_laxis*width,height_vis*0.05,text=tafhour,fill=axis_color)
			self.cv.create_line(i*pct_graph_width*width/24+pct_width_laxis*width, pct_top*height_vis,
			i*pct_graph_width*width/24+pct_width_laxis*width,pct_bot*height_vis,fill=axis_color,dash=(4,4))
	
	# y-axis set-up	
		for index,i in enumerate(range(len(visbys))):
			j = float(visbys[i])			
			test = abs(graph_height_vis-((graph_height_vis*math.log10(j)/2)+graph_height_vis/2.0))+pct_top*height_vis
...
# Controls ##########################		
		...
		self.taflabel = tk.Label(self.Frame_right,text="Click on graphs \nto generate TAF\n\nTAF will show up here after\n you have both visibility and ceilings.\n\n1) Left click will place points\n2) Middle click to drag/move\n3) Right click to delete",bd=3,bg='white',relief="raised",anchor=tk.W,justify=tk.LEFT)
	...
	####################
		self.sitelabel = tk.Label(self.Frame,text="Site:",bd=3,fg='#005306',relief="flat",anchor=tk.W,justify=tk.LEFT)
		self.sitelabel.grid( row = 0,column=0,sticky=tk.W,padx=10)
		
		self.ciglabel = tk.Label(self.Frame,text="Ceiling:",bd=3,fg='#005306',relief="flat",anchor=tk.W,justify=tk.LEFT)
		...
		wxarr = ['',"FG","BR","FZFG","-RA","RA","+RA","-SN","SN","+SN","-DZ","DZ","+DZ","-TSRA","TSRA","+TSRA"]		
		...
		for index,site in enumerate(sites):
			self.sitelabels[site]=tk.Label(self.Frame_right,text=" "+site+" ",bd=2,fg='black',bg='#B2B4A8',relief="raised",anchor=tk.W,justify=tk.LEFT)
	...
	def combineTAF(self):
		...
		for i in sites:
			try:
				...
				ok=1
		if ok ==1:
			ok = tkMessageBox.showinfo("Ok","There are some sites you haven't saved yet.")
	...
	def formatTime(self,graphtime):		
		...
		(year,month,tafday,tafhour,tmin,jsec,wday,yday,dst) = time.gmtime(time.mktime(self.tafpackage)+int(graphtime)*3600)	
		if tafhour<10:
	...
	def labelTaf(self):
		...
		for index,i in enumerate(mytaf):
			if index==0:
				taf = "TAF\n"+self.sitepick.get()+' '+self.lt10(day)+self.lt10(jhour)+self.lt10(jmin)+"Z "+self.lt10(day)+self.lt10(self.tafphour)+"/04"+self.lt10(self.tafphour)+" 00000KT "
				...
				if mycig!='SKC':
					taf=taf+'   FM'+mytaf[index]+'00 '+ mywind +'KT ' +myvis+' '+self.skypick.get()+mycig+"\n"
				else:	
	...
	def toGraphCoordfromTAF_vis(self,x,y):
		return x*pct_graph_width*width/24+pct_width_laxis*width, abs(graph_height_vis-((graph_height_vis*math.log10(y)/2)+graph_height_vis/2.0))+pct_top*height_vis
	...
	def fromGraphCoordtoTAF_vis(self,x,y):
		...
		#print y,exponent,math.pow(10,exponent)
		return ((x-pct_width_laxis*width)*24)/(pct_graph_width*width), math.pow(10,exponent)	
	
	def toGraphCoordfromTAF(self,x,y):
		return x*pct_graph_width*width/24+pct_width_laxis*width, abs(graph_height-(graph_height*(math.log10(y)-2)/2))+pct_top*height
	...
	def motion_vis (self, event):
  	...
  	def up (self, event):
		...
		if (self.target ==event.widget.find_withtag (tk.CURRENT)):
			pass #print #"Select %s" %event.widget.itemcget (tk.CURRENT, "text")
		else:
...
# Importing TAF ####################################################################################################	
	...
	def plotTAF(self):
		...
		for index,i in enumerate(self.taf[_id]['fxtime']):
			...
			#if type(temptime).__name__=='list' and len(temptime)>=1:
				#temptime=temptime[0]
	...
	def detCig(self,line):
		...
		if date:
			...
			#(_year,_month,_day,_hour,_min,_sec,_wday,_yday,_dst) = time.gmtime(time.mktime(self.tafpackage)+86400)
		...
		#print self.begin_month, self.begin_day, self.begin_year , self.end_month,self.end_day, self.end_year	 
		...
		if t:
			#time tafs were sent (after 30 after the hour increment up)
			#print 'yikes',t,t[0][5]
			#if t[0][5]>3:
				#ti = int(t[0][0:4])+1
			#else:
			...
			hour_fm_tafbegin = (time.mktime( time.strptime(self.begin_year+self.begin_month+self.lt10(ti),"%Y%m%d%H"))-time.mktime(time.strptime(self.begin_year+self.begin_month+self.tafbegin_group,"%Y%m%d%H")))/3600
			#print 'here',self.begin_year+self.begin_month+self.lt10(ti),self.begin_year+self.begin_month+self.tafbegin_group
			#if abs(hour_fm_tafbegin) > 24:
				#hour_fm_tafbegin =  (time.mktime( time.strptime(self.end_year+self.end_month+str(ti),"%Y%m%d%H"))-time.mktime(time.strptime(self.begin_year+self.begin_month+self.tafbegin_group,"%Y%m%d%H")))/3600
			#print "hour_fm_tafbegin",hour_fm_tafbegin
		...
		if ttime:	
			...
			hour_fm_tafbegin = (time.mktime( time.strptime(self.begin_year+self.begin_month+ttime[0][0:4],"%Y%m%d%H"))-time.mktime(time.strptime(self.begin_year+self.begin_month+self.tafbegin_group,"%Y%m%d%H")))/3600
			
			if abs(hour_fm_tafbegin) > 24:
				hour_fm_tafbegin =  (time.mktime( time.strptime(self.end_year+self.end_month+ttime[0][0:4],"%Y%m%d%H"))-time.mktime(time.strptime(self.begin_year+self.begin_month+self.tafbegin_group,"%Y%m%d%H")))/3600
			#print
			#print "hour_fm_tafbegin",hour_fm_tafbegin
			#print self.begin_year+self.begin_month+ttime[0][0:4],self.begin_year+self.begin_month+self.tafbegin_group
